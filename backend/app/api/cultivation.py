...
注意：
1. 每个 item 必须包含 reason 字段（50-150字），具体说明基于用户的哪些信息推测出该值
   - 例如："基于你的职业为软件工程师、本科学历、O型血和双子座特质，推测..."
   - 可以引用：职业、学历、年龄、血型、星座、性别、婚姻、收入、MBTI、大五人格、昼夜节律、决策风格、人际互动
2. 每个 item 的 options 数组必须包含 3-6 个选项（可以是空数组表示自由填写）
3. confidence 范围 0.1-0.9，AI 不确定的项给低置信度
4. 职业相关的项（职业身份、经济资源）confidence 应较高
5. 只返回 JSON，不要其他文字
"""
